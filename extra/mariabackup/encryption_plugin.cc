...
static std::string get_encryption_plugin_from_cnf()
{
  ...
  if (!f)
  {
    msg_ts("Can't open backup-my.cnf for reading");
    exit(EXIT_FAILURE);
  }
  char line[512];
...
void encryption_plugin_prepare_init(int argc, char **argv)
{
  ...
  if (plugin_load.size())
  {
    msg_ts("Loading encryption plugin from %s", plugin_load.c_str());
  }
  else
  {
...
static void encryption_plugin_init(int argc, char **argv)
{
  ...
  msg_ts("Loading encryption plugin");
  for (int i= 1; i < argc; i++)
    msg_ts("\t Encryption plugin parameter :  '%s'", argv[i]);
