	...
	while (list != NULL) {
		...
		* mystat struct */
		...
		dst_file = ds_open(pipe_ctxt, tmp_file->orig_path,
				   &tmp_file->mystat);
		if (dst_file == NULL) {
			msg_ts("error: could not stream a temporary file to "
		...
		if (my_seek(tmp_file->fd, 0, SEEK_SET, MYF(0)) ==
		    MY_FILEPOS_ERROR) {
			msg_ts("error: my_seek() failed for '%s', errno = %d.",
		...
		while ((bytes = my_read(tmp_file->fd, (unsigned char *)buf, buf_size,
			...
			if (ds_write(dst_file, buf, bytes)) {
				msg_ts("error: cannot write to stream for '%s'.",
				    tmp_file->orig_path);
				exit(EXIT_FAILURE);
			}
		}
