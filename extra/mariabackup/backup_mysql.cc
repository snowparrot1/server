...
xb_mysql_connect()
{
	...
	if (connection == NULL) {
		msg_ts("Failed to init MySQL struct: %s.",
	...
	if (!mysql_real_connect(connection,
				...
				"" /*database*/, opt_port,
				opt_socket, 0)) {
		msg_ts("Failed to connect to MySQL server: %s.", mysql_error(connection));
		...
		bool die_on_error)
{
	...
	if (mysql_query(connection, query)) {
		msg_ts("Error: failed to execute query %s: %s", query, mysql_error(connection));
		if (die_on_error) {
			exit(EXIT_FAILURE);
		}
		return(NULL);
	...
	if (mysql_field_count(connection) > 0) {
		if ((mysql_result = mysql_store_result(connection)) == NULL) {
			msg_ts("Error: failed to fetch query result %s: %s",
...
get_mysql_vars(MYSQL *connection)
{
	...
	if (have_backup_safe_binlog_info_var == NULL &&
		...
		msg_ts("Error: --binlog-info=LOCKLESS is not supported by the "
		    "server");
		return(false);
	}
	...
	msg_ts("Using server version %s", version_var);
...
detect_mysql_capabilities_for_backup()
{
	...
	if (opt_galera_info && !have_galera_enabled) {
		msg_ts("--galera-info is specified on the command "
	...
	if (opt_slave_info && have_multi_threaded_slave &&
	    !have_gtid_slave) {
	    	msg_ts("The --slave-info option requires GTID enabled for a "
...
select_incremental_lsn_from_history(lsn_t *incremental_lsn)
{
	...
	if (row) {
		*incremental_lsn = strtoull(row[0], NULL, 10);
		msg_ts("Found and using lsn: " LSN_PF " for %s %s",
		    *incremental_lsn,
		...
		msg_ts("Error while attempting to find history record "
			"for %s %s",
			opt_incremental_history_uuid ? "uuid" : "name",
			opt_incremental_history_uuid ?
		    		opt_incremental_history_uuid :
	...
	void *arg __attribute__((unused)))
{
	...
	if ((mysql = xb_mysql_connect()) == NULL) {
		msg_ts("Error: kill query thread failed");
		goto stop_thread;
	}
...
DECLARE_THREAD(kill_mdl_waiters_thread(void *))
{
	MYSQL	*mysql;
	if ((mysql = xb_mysql_connect()) == NULL) {
		msg_ts("Error: kill mdl waiters thread failed to connect");
		goto stop_thread;
	}
	...
	for(;;){
		...
		while (MYSQL_ROW row = mysql_fetch_row(result))
		{
			...
			if (mysql_query(mysql, query) && (mysql_errno(mysql) != ER_NO_SUCH_THREAD)) {
				msg_ts("Error: failed to execute query %s: %s", query,mysql_error(mysql));
				exit(EXIT_FAILURE);
			}
		}
		mysql_free_result(result);
...
wait_for_safe_slave(MYSQL *connection)
{
	...
	if (!(read_master_log_pos && slave_sql_running)) {
		msg_ts("Not checking slave open temp tables for "
	...
	msg_ts("Slave open temp tables: %d", open_temp_tables);
...
write_slave_info(MYSQL *connection)
{
	...
	if (master == NULL || filename == NULL || position == NULL) {
		msg_ts("Failed to get master binlog coordinates "
...
write_galera_info(MYSQL *connection)
{
	...
	if ((state_uuid == NULL && state_uuid55 == NULL)
		...
		msg_ts("Failed to get master wsrep state from SHOW STATUS.");
		result = false;
		goto cleanup;
	}
...
write_current_binlog_file(MYSQL *connection)
{
	...
	if (gtid_exists) {
		...
		if (log_bin_dir == NULL || log_bin_file == NULL) {
			msg_ts("Failed to get master binlog coordinates from "
				"SHOW MASTER STATUS");
			result = false;
			goto cleanup;
		}
...
mdl_lock_init()
{
  ...
  if (!mdl_con)
  {
    msg_ts("FATAL: cannot create connection for MDL locks");
    exit(1);
  }
  const char *query =
...
mdl_lock_table(ulint space_id)
{
  ...
  msg_ts("Locking MDL for %s", full_table_name.c_str());
  if (mysql_query(mdl_con, lock_query.str().c_str())) {
      msg_ts("Warning : locking MDL failed for space id %zu, name %s", space_id, full_table_name.c_str());
  } else {
