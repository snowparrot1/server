	...
	ulonglong max_file_size)
{
	...
	if (!node->is_open()) {
		...
		if (!success) {
			...
			msg_ts(thread_n, "mariabackup: error: cannot open "
	...
	if (err) {
		msg_ts(thread_n, "mariabackup: error: cannot fstat %s",
		    cursor->abs_path);
	...
	xb_fil_cur_t*	cursor)	/*!< in/out: source file cursor */
{
	...
	if ((to_read & ~(page_size - 1))
		...
		if (to_read < (ib_int64_t) page_size) {
			msg_ts(cursor->thread_n, "Warning: junk at the end of "
...
read_retry:
	...
	for (page = cursor->buf, i = 0; i < npages;
		...
		if (page_is_corrupted(page, page_no, cursor, space)){
			...
			msg_ts(cursor->thread_n, "Database page corruption detected at page "
			    ULINTPF ", retrying...", 
			    page_no);
