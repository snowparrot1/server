...
datadir_iter_next_database(datadir_iter_t *it)
{
	...
	while (os_file_readdir_next_file(it->datadir_path,
		...
		if (check_if_skip_database_by_path(it->dbpath)) {
			msg_ts("Skipping db: %s", it->dbpath);
			continue;
		}
...
datafile_open(const char *file, datafile_cur_t *cursor, uint thread_n)
{
	...
	if (!success) {
		...
		msg_ts(thread_n,"error: cannot open "
	...
	if (!my_stat(cursor->abs_path, &cursor->statinfo, 0)) {
		msg_ts(thread_n, "error: cannot stat %s", cursor->abs_path);
...
directory_exists(const char *dir, bool create)
{
	...
	if (my_stat(dir, &stat_arg, MYF(0)) == NULL) {
		...
		if (mkdirp(dir, 0777, MYF(0)) < 0) {
			...
			msg_ts("Can not create directory %s: %s", dir, errbuf);
			return(false);
		}
	}
	...
	if (os_dir == NULL) {
		my_strerror(errbuf, sizeof(errbuf), my_errno);
		msg_ts("Can not open directory %s: %s", dir,
			errbuf);
...
directory_exists_and_empty(const char *dir, const char *comment)
{
	...
	if (os_dir == NULL) {
		msg_ts("%s can not open directory %s", comment, dir);
		return(false);
	}
	...
	if (!empty) {
		msg_ts("%s directory %s is not empty!", comment, dir);
	}
...
datafile_copy_backup(const char *filepath, uint thread_n)
{
	...
	if (check_if_skip_table(filepath)) {
		msg_ts(thread_n,"Skipping %s.", filepath);
		return(true);
	}
...
backup_file_vprintf(const char *filename, const char *fmt, va_list ap)
{
	...
	if (dstfile == NULL) {
		msg_ts("error: Can't open the destination stream for %s",
			filename);
		goto error;
	}

	action = xb_get_copy_action("Writing");
	msg_ts("%s %s", action, filename);
...
error_close:
	msg_ts("Error: backup file failed.");
	return(false); /*ERROR*/
}
...
run_data_threads(datadir_iter_t *it, os_thread_func_t func, uint n)
{
	...
	for (i = 0; i < n; i++) {
		...
		if (!data_threads[i].ret) {
			msg_ts("Error: thread %u failed.", i);
		}
	}
	  ...
	  uint thread_n)
{
	...
	if (dstfile == NULL) {
		msg_ts(thread_n,"error: "
			"cannot open the destination stream for %s", dst_name);
		goto error;
	}

	msg_ts(thread_n, "%s %s to %s", xb_get_copy_action(), src_file_path, dstfile->path);
...
error_close:
	msg_ts(thread_n,"Error: copy_file() failed.");
	return(false); /*ERROR*/
}
	  ...
	  const char *dst_dir, uint thread_n)
{
	...
	if (file_exists(dst_file_path_abs)) {
		msg_ts("Error: Move file %s to %s failed: Destination "
			"file exists", src_file_path, dst_file_path_abs);
		return(false);
	}

	msg_ts(thread_n,"Moving %s to %s", src_file_path, dst_file_path_abs);
	...
	if (my_rename(src_file_path, dst_file_path_abs, MYF(0)) != 0) {
		if (my_errno == EXDEV) {
			...
			msg_ts(thread_n,"Removing %s", src_file_path);
			if (unlink(src_file_path) != 0) {
				my_strerror(errbuf, sizeof(errbuf), errno);
				msg_ts("Error: unlink %s failed: %s",
					src_file_path,
					errbuf);
			}
			return(ret);
		...
		my_strerror(errbuf, sizeof(errbuf), my_errno);
		msg_ts("Can not move file %s to %s: %s",
...
backup_files(const char *from, bool prep_mode)
{
	...
	if (opt_rsync) {
		...
		if (rsync_tmpfile == NULL) {
			msg_ts("Error: can't create file %s",
				rsync_tmpfile_name);
			return(false);
		}
	}
	...
	while (datadir_iter_next(it, &node)) {
		...
		if (!node.is_empty_dir) {
			...
			if (!ret) {
				msg_ts("Failed to copy file %s", node.filepath);
				goto out;
			}
		} else if (!prep_mode) {
			...
			if (!(ret = backup_file_printf(
				...
				msg_ts("Failed to create file %s", path);
				goto out;
			}
		}
	}
	...
	if (opt_rsync) {
		...
		msg_ts("Starting rsync as: %s", cmd.str().c_str());
		if ((err = system(cmd.str().c_str()) && !prep_mode) != 0) {
			msg_ts("Error: rsync failed with error code %d", err);
			ret = false;
			goto out;
		}
		...
		if (!prep_mode && !opt_no_lock) {
			...
			if (rsync_tmpfile == NULL) {
				msg_ts("Error: can't open file %s",
					rsync_tmpfile_name);
				ret = false;
				goto out;
			}
...
bool backup_start()
{
	...
	msg_ts("Waiting for log copy thread to read lsn %llu", (ulonglong)server_lsn_after_lock);
...
void backup_release()
{
	...
	if (opt_safe_slave_backup && sql_thread_started) {
		msg_ts("Starting slave SQL thread");
		xb_mysql_query(mysql_connection,
...
bool backup_finish()
{
	...
	msg_ts("Backup created in directory '%s'", xtrabackup_target_dir);
	if (mysql_binlog_position != NULL) {
		msg_ts("MySQL binlog position: %s", mysql_binlog_position);
	}
	if (mysql_slave_position && opt_slave_info) {
		msg_ts("MySQL slave binlog position: %s",
...
copy_back()
{
	...
	{
		msg_ts("Can't my_setwd %s", xtrabackup_target_dir);
		return(false);
	}
	...
	if (!srv_sys_space.parse_params(innobase_data_file_path, true)) {
		msg_ts("syntax error in innodb_data_file_path");
		return(false);
	}
	...
	while (datadir_iter_next(it, &node)) {
		...
		if (node.is_empty_dir) {
			...
			msg_ts("Creating directory %s", path);

			if (mkdirp(path, 0777, MYF(0)) < 0) {
				char errbuf[MYSYS_STRERROR_SIZE];
				my_strerror(errbuf, sizeof(errbuf), my_errno);
				msg_ts("Can not create directory %s: %s",
					path, errbuf);
				ret = false;
	...
	cmd << IF_WIN("type ","cat ") << filepath;
 	...
 	if (needs_action) {
		...
		if (opt_remove_original) {
			msg_ts(thread_n, "Removing %s", filepath);
...
static void copy_or_move_dir(const char *from, const char *to, bool do_copy, bool allow_hardlinks)
{
	...
	while (datadir_iter_next(it, &node))
	{
		...
		if (do_copy && allow_hardlinks)
		{
			...
			if (rc)
			{
				msg_ts("Creating hardlink from %s to %s",from_path, to_path);
			}
			else
			{
