...
main(int argc, char **argv)
{
	...
	if (opt_mode == RUN_MODE_NONE) {
		msg_ts("%s: either -c or -x must be specified.", my_progname);
		goto err;
	}
...
{
	if (opt_mode != RUN_MODE_NONE) {
		msg_ts("%s: can't set specify both -c and -x.", my_progname);
		return 1;
	}
...
stream_one_file(File file, xb_wstream_file_t *xbfile)
{
	...
	while ((bytes = (ssize_t)my_read(file, buf, XBSTREAM_BUFFER_SIZE,
		...
		if (xb_stream_write_data(xbfile, buf, bytes)) {
			msg_ts("%s: xb_stream_write_data() failed.",
			    my_progname);
			my_free(buf);
			return 1;
		}
...
mode_create(int argc, char **argv)
{
	...
	if (argc < 1) {
		msg_ts("%s: no files are specified.", my_progname);
		return 1;
	}

	stream = xb_stream_write_new();
	if (stream == NULL) {
		msg_ts("%s: xb_stream_write_new() failed.", my_progname);
		return 1;
	}
	...
	for (i = 0; i < argc; i++) {
		...
		if (!MY_S_ISREG(mystat.st_mode)) {
			msg_ts("%s: %s is not a regular file, exiting.",
			    my_progname, filepath);
			goto err;
		}

		if ((src_file = my_open(filepath, O_RDONLY, MYF(MY_WME))) < 0) {
			msg_ts("%s: failed to open %s.", my_progname, filepath);
			goto err;
		}
...
file_entry_new(extract_ctxt_t *ctxt, const char *path, uint pathlen)
{
	...
	if (file == NULL) {
		msg_ts("%s: failed to create file.", my_progname);
		goto err;
	}
	...
	while (1) {
		...
		if (entry == NULL) {
			...
			if (my_hash_insert(ctxt->filehash, (uchar *) entry)) {
				msg_ts("%s: my_hash_insert() failed.",
				    my_progname);
				pthread_mutex_unlock(ctxt->mutex);
				break;
			}
		...
		if (entry->offset != chunk.offset) {
			msg_ts("%s: out-of-order chunk: real offset = 0x%llx, "
		...
		if (ds_write(entry->file, chunk.data, chunk.length)) {
			msg_ts("%s: my_write() failed.", my_progname);
			pthread_mutex_unlock(&entry->mutex);
			res = XB_STREAM_READ_ERROR;
			break;
		}
	     ...
	     char **argv __attribute__((unused)))
{
	...
	if (my_hash_init(&filehash, &my_charset_bin, START_FILE_HASH_SIZE,
		...
		msg_ts("%s: failed to initialize file hash.", my_progname);
		return 1;
	}

	if (pthread_mutex_init(&mutex, NULL)) {
		msg_ts("%s: failed to initialize mutex.", my_progname);
