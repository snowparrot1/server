	...
	const byte* new_name, ulint new_len)
{
	...
	if (fail) {
		msg_ts("ERROR : DDL operation detected in the late phase of backup."
...
static void backup_optimized_ddl_op_fail(ulint space_id) {
	ut_a(opt_no_lock);
	msg_ts("DDL tracking : optimized DDL on space %zu", space_id);
	if (ddl_tracker.tables_in_backup.find(space_id) != ddl_tracker.tables_in_backup.end()) {
		msg_ts("ERROR : Optimized DDL operation detected in the late phase of backup."
			"Backup is inconsistent. Remove --no-lock option to fix.");
		exit(EXIT_FAILURE);
	}
}
...
{
	msg_ts("mariabackup: Incompatible TRUNCATE operation detected.%s",
...
debug_sync_point(const char *name)
{
	...
	fp = fopen(pid_path, "w");
	if (fp == NULL) {
		msg_ts("mariabackup: Error: cannot open %s", pid_path);
		exit(EXIT_FAILURE);
	}
	fprintf(fp, "%u\n", (uint) pid);
	fclose(fp);

	msg_ts("mariabackup: DEBUG: Suspending at debug sync point '%s'. "
	...
	/* On resume */
	msg_ts("mariabackup: DEBUG: removing the pid file.");
...
static int prepare_export()
{
  ...
  msg_ts("Prepare export : executing %s\n", cmdline);
  fflush(stderr);
...
{
  msg_ts("%s based on MariaDB server %s %s (%s)",
		  ...
		  char *argument)
{
  ...
  case OPT_XTRA_STREAM:
    ...
    {
      msg_ts("Invalid --stream argument: %s", argument);
      return 1;
    }
  ...
  case OPT_XTRA_COMPRESS:
    ...
    {
      msg_ts("Invalid --compress argument: %s", argument);
      return 1;
    }
...
innodb_init_param(void)
{
	...
	if (innobase_page_size != (1LL << 14)) {
		...
		if (n_shift >= 12 && n_shift <= UNIV_PAGE_SIZE_SHIFT_MAX) {
			...
			msg_ts("InnoDB: The page size of the "
			    "database is set to %lu.\n", srv_page_size);
		} else {
			msg_ts("InnoDB: Error: invalid value of "
	...
	if (sizeof(ulint) == 4) {
		if (xtrabackup_use_memory > UINT_MAX32) {
			msg_ts("mariabackup: use-memory can't be over 4GB"
			    " on 32-bit systems");
		}

		if (innobase_buffer_pool_size > UINT_MAX32) {
			msg_ts("mariabackup: innobase_buffer_pool_size can't be "
	...
	if (xtrabackup_backup) {
		msg_ts("mariabackup: using the following InnoDB configuration:");
	} else {
		msg_ts("mariabackup: using the following InnoDB configuration "
		    "for recovery:");
	}
	...
	msg_ts("innodb_data_home_dir = %s", srv_data_home);
	...
	msg_ts("innodb_data_file_path = %s",
	...
	msg_ts("innodb_log_group_home_dir = %s",
	    srv_log_group_home_dir);

	os_normalize_path(srv_log_group_home_dir);

	if (strchr(srv_log_group_home_dir, ';')) {
		msg_ts("syntax error in innodb_log_group_home_dir, ");
		goto error;
	}
	...
	if (srv_use_native_aio) {
		msg_ts("InnoDB: Using Linux native AIO");
	}
#else
...
error:
	msg_ts("innodb_init_param(): Error occured.");
	return(TRUE);
}
...
static bool innodb_init()
{
	...
	if (err != DB_SUCCESS) {
		msg_ts("mariabackup: innodb_init() returned %d (%s).",
		    err, ut_strerr(err));
		innodb_shutdown();
		return(TRUE);
	}
...
xtrabackup_read_metadata(char *filename)
{
	...
	fp = fopen(filename,"r");
	if(!fp) {
		msg_ts("Error: cannot open %s", filename);
		return(FALSE);
	}
...
xtrabackup_stream_metadata(ds_ctxt_t *ds_ctxt)
{
	...
	if (stream == NULL) {
		msg_ts("Error: cannot open output stream for %s", XTRABACKUP_METADATA_FILENAME);
		return(FALSE);
	}
...
xtrabackup_write_metadata(const char *filepath)
{
	...
	fp = fopen(filepath, "w");
	if(!fp) {
		msg_ts("Error: cannot open %s", filepath);
		return(FALSE);
	}
...
xb_read_delta_metadata(const char *filepath, xb_delta_info_t *info)
{
	...
	if (page_size == ULINT_UNDEFINED) {
		msg_ts("page_size is required in %s", filepath);
		r = FALSE;
	} else {
	...
	if (info->space_id == ULINT_UNDEFINED) {
		msg_ts("mariabackup: Warning: This backup was taken with XtraBackup 2.0.1 "
...
xb_write_delta_metadata(const char *filename, const xb_delta_info_t *info)
{
	...
	if (f == NULL) {
		msg_ts("Error: Can't open output stream for %s",filename);
		return(FALSE);
	}
...
xtrabackup_copy_datafile(fil_node_t* node, uint thread_n, const char *dest_name=0, ulonglong max_size=ULLONG_MAX)
{
	...
	if (fil_is_user_tablespace_id(node->space->id)
		...
		msg_ts(thread_n, "Skipping %s.", node_name);
		return(FALSE);
	}
	...
	if (write_filter->init != NULL &&
		...
		msg_ts (thread_n, "mariabackup: error: failed to initialize page write filter.");
		goto error;
	}

	dstfile = ds_open(ds_data, dst_name, &cursor.statinfo);
	if (dstfile == NULL) {
		msg_ts(thread_n,"mariabackup: error: can't open the destination stream for %s", dst_name);
		goto error;
	}
	...
	if (xtrabackup_stream) {
		msg_ts(thread_n, "%s %s", action, node_path);
	} else {
		msg_ts(thread_n, "%s %s to %s", action, node_path, dstfile->path);
	}
...
error:
	...
	msg_ts(thread_n, "mariabackup: xtrabackup_copy_datafile() failed.");
	return(TRUE); /*ERROR*/
...
skip:
	...
	msg_ts(thread_n,"Warning: We assume the  table was dropped during xtrabackup execution and ignore the tablespace %s", node_name);
	return(FALSE);
}
...
static lsn_t xtrabackup_copy_log(lsn_t start_lsn, lsn_t end_lsn, bool last)
{
	...
	for (ulint scanned_checkpoint = 0;
		...
		if (scanned_checkpoint > checkpoint
			...
			msg_ts(0,"checkpoint wrap: " LSN_PF ",%zx,%zx",
		...
		if (data_len == OS_FILE_LOG_BLOCK_SIZE) {
			...
			msg_ts(0,"garbage block: " LSN_PF ",%zu",scanned_lsn, data_len);
			break;
		} else {
	...
	if (more_data && recv_parse_log_recs(0, STORE_NO, false)) {
		...
		msg_ts("Error: copying the log failed");
	...
	if (ulint write_size = ulint(end_lsn - start_lsn)) {
		...
		if (ds_write(dst_log_file, log_sys->buf, write_size)) {
			msg_ts("Error: write to logfile failed\n");
			return(0);
		}
	}
...
static bool xtrabackup_copy_logfile(bool last = false)
{
	...
	do {
		...
		for (int retries= 0; retries < 100; retries++) {
			...
			msg_ts("Retrying read of log at LSN=" LSN_PF, lsn);
			my_sleep(1000);
		}
		...
		if (!start_lsn) {
			msg_ts("Error: xtrabackup_copy_logfile() failed.");
			exit(EXIT_FAILURE);
		}
	...
	msg_ts(">> log scanned up to (" LSN_PF ")", start_lsn);
...
static void dbug_mariabackup_event(const char *event,const char *key)
{
	...
	if (sql) {
		msg_ts("dbug_mariabackup_event : executing '%s'", sql);
	...
	*/
	...
	while ((node = datafiles_iter_next(ctxt->it)) != NULL) {
		...
		if(xtrabackup_copy_datafile(node, num)) {
			msg_ts(num,"mariabackup: Error: failed to copy datafile.");
			exit(EXIT_FAILURE);
		}
	...
	bool is_remote)
{
	...
	if (file->open_read_only(true) != DB_SUCCESS) {
		msg_ts("Can't open datafile %s", name);
		ut_free(name);
		exit(EXIT_FAILURE);
	}
...
static dberr_t xb_assign_undo_space_start()
{
	...
	if (!ret) {
		msg_ts("Error opening %s", srv_sys_space.first_datafile()->filepath());
		return DB_ERROR;
	}
...
retry:
	if (!os_file_read(IORequestRead, file, page, TRX_SYS_PAGE_NO * UNIV_PAGE_SIZE,
			  UNIV_PAGE_SIZE)) {
		msg_ts("Reading TRX_SYS page failed.");
		error = DB_ERROR;
		goto func_exit;
	}
	...
	if (buf_page_is_corrupted(false, page, univ_page_size)) {
		if (n_retries--) {
			...
			msg_ts("mariabackup: TRX_SYS page corrupted.\n");
			error = DB_ERROR;
			goto func_exit;
		}
	}
...
xb_load_tablespaces()
{
	...
	if (err != DB_SUCCESS || create_new_db) {
		msg_ts("Could not find data files at the specified datadir");
		return(DB_ERROR);
	}
	...
	if (err != DB_SUCCESS) {
		msg_ts("Could not open data files.\n");
		return(err);
	}
	...
	msg_ts("mariabackup: Generating a list of tablespaces");
	...
	size_t		len)	/*!< in: length of name */
{
	...
	path symbols */
	if (len > NAME_LEN) {
		msg_ts("mariabackup: name `%s` is too long.", name);
		exit(EXIT_FAILURE);
	}
	p = strpbrk(name, "/\\~");
	if (p && (uint) (p - name) < NAME_LEN) {
		msg_ts("mariabackup: name `%s` is not valid.", name);
		exit(EXIT_FAILURE);
	}
}
...
{
	if (strchr(name, '.') == NULL) {
		msg_ts("mariabackup: `%s` is not fully qualified name.", name);
		exit(EXIT_FAILURE);
	}
	...
	regex_list_t* list) /*! in: list to put new regex to */
{
	...
	if (ret != 0) {
		...
		msg_ts("mariabackup: error: %s regcomp(%s): %s",
	...
	insert_entry_func_t ins)	/*!< in: callback to add entry */
{
	...
	fp = fopen(filename, "r");
	if (!fp) {
		msg_ts("mariabackup: cannot open %s",
		    filename);
		exit(EXIT_FAILURE);
	}
	while (fgets(name_buf, sizeof(name_buf), fp) != NULL) {
		...
		if (p) {
			*p = '\0';
			...
			msg_ts("mariabackup: `%s...` name is too long", name_buf);
			exit(EXIT_FAILURE);
		}
...
static bool xtrabackup_backup_low()
{
	...
	/* read the latest checkpoint lsn */
	{
		...
		if (recv_find_max_checkpoint(&max_cp_field) == DB_SUCCESS
			...
			msg_ts("The latest check point"
			    " (for incremental): '" LSN_PF "'",
			    metadata_to_lsn);
		} else {
			msg_ts("Error: recv_find_max_checkpoint() failed.");
		}
		log_mutex_exit();
	}
	...
	if (!xtrabackup_stream_metadata(ds_meta)) {
		msg_ts("Error: failed to stream metadata.");
		return false;
	}
	if (xtrabackup_extra_lsndir) {
		...
		if (!xtrabackup_write_metadata(filename)) {
			msg_ts("Error: failed to write metadata "
			    "to '%s'.", filename);
			return false;
		}
		sprintf(filename, "%s/%s", xtrabackup_extra_lsndir,
		...
		if (!write_xtrabackup_info(mysql_connection, filename, false)) {
			msg_ts("Error: failed to write info "
			 "to '%s'.", filename);
			return false;
		}
	}
...
xtrabackup_backup_func()
{
	...
	{
		msg_ts("my_setwd() failed , %s", mysql_real_data_home);
		return(false);
	}
	msg_ts("cd to %s", mysql_real_data_home);
	encryption_plugin_backup_init(mysql_connection);
	msg_ts("open files limit requested %u, set to %u",
...
fail:
	...
	if (srv_file_flush_method_str == NULL) {
		...
		msg_ts("using ALL_O_DIRECT");
	} else if (0 == ut_strcmp(srv_file_flush_method_str,
				  "O_DIRECT_NO_FSYNC")) {
		srv_file_flush_method = SRV_O_DIRECT_NO_FSYNC;
		msg_ts("using O_DIRECT_NO_FSYNC");
	} else {
		msg_ts("Unrecognized value %s for "
...
old_format:
		msg_ts("Error: cannot process redo log"
...
reread_log_header:
	...
	if (dst_log_file == NULL) {
		msg_ts("§rror: failed to open the target stream for "
		    "'ib_logfile0'.");
		goto fail;
	}
	...
	if (err != DB_SUCCESS) {
		msg_ts("merror: xb_load_tablespaces() failed with"
...
fail_before_log_copying_thread_start:
	...
	if (xtrabackup_parallel > 1) {
		msg_ts("mariabackup: Starting %u threads for parallel data "
	...
	datafiles_iter_t *it = datafiles_iter_new(fil_system);
	if (it == NULL) {
		msg_ts("mariabackup: Error: datafiles_iter_new() failed.");
		goto fail;
	}
	...
	msg_ts("Redo log (from LSN " LSN_PF " to " LSN_PF
	...
	if (metadata_to_lsn > log_copy_scanned_lsn) {
		msg_ts("Error: failed to copy enough redo log ("
...
void backup_fix_ddl(void)
{
	...
	for (std::set<std::string>::iterator iter = new_tables.begin();
		...
		char *p = strchr(buf, '/');
		if (p == 0) {
			msg_ts("Unexpected tablespace %s filename %s", space_name, name.c_str());
			ut_a(0);
		}
		ut_a(p);
		*p = 0;
	...
	pfs_os_file_t*	file)		/*!<out: file handle */
{
	...
	*file = os_file_create_simple_no_error_handling(
	...
	if (!ret) {
		msg_ts("Can't create file %s", path);
		return ret;
	}
	...
	if (!ret) {
		msg_ts("mariabackup: cannot set size for file %s", path);
		os_file_close(*file);
	...
	if (!ret) {
		msg_ts("mariabackup: could not write the first page to %s",
		    path);
		os_file_close(*file);
		os_file_delete(0, path);
		return ret;
	...
	bool* 		success)	/* out: indicates error. true = success */
{
	...
	*success = false;
	...
	if (!os_file_create_directory(dest_dir, FALSE)) {
		msg_ts("mariabackup: error: cannot create dir %s", dest_dir);
		return file;
	}
...
found:
		...
		if (!*success) {
			msg_ts("mariabackup: Cannot open file %s\n", real_name);
		}
exit:
		log_mutex_exit();
		return file;
	...
	if (fil_space != NULL) {
		if (fil_space->id == info.space_id
			...
			msg_ts("mariabackup: Renaming %s to %s.ibd",
			...
			if (!fil_rename_tablespace(
				...
				tmpname, NULL))
			{
				msg_ts("mariabackup: Cannot rename %s to %s",
					fil_space->name, tmpname);
				goto exit;
			}
		}
	...
	if (info.space_id == ULINT_UNDEFINED)
	{
		msg_ts("mariabackup: Error: Cannot handle DDL operation on tablespace "
		    "%s\n", dest_space_name);
		exit(EXIT_FAILURE);
	}
	mutex_enter(&fil_system->mutex);
	...
	if (fil_space != NULL) {
		...
		msg_ts("mariabackup: Renaming %s to %s",
	...
	if (fil_space_create(dest_space_name, info.space_id, flags,
		...
		*success = xb_space_create_file(real_name, info.space_id,
		...
		msg_ts("Can't create tablespace %s\n", dest_space_name);
	}
	...
	void*		/*data*/)
{
	...
	msg_ts("page size for %s is %zu bytes",
	    src_path, page_size);
	if (page_size_shift < 10 ||
	    page_size_shift > UNIV_PAGE_SIZE_SHIFT_MAX) {
		msg_ts("error: invalid value of page_size "
	...
	if (!success) {
		...
		msg_ts("error: can't open %s", src_path);
		goto error;
	}
	...
	if (!success) {
		msg_ts("error: can't open %s", dst_path);
		goto error;
	}
	...
	msg_ts("Applying %s to %s...", src_path, dst_path);
...
error:
	...
	if (dst_file != OS_FILE_CLOSED)
		os_file_close(dst_file);
	msg_ts("Error: xtrabackup_apply_delta(): "
...
static void rename_file(const char *from,const char *to) {
	msg_ts("Renaming %s to %s\n", from, to);
	if (my_rename(from, to, MY_WME)) {
		msg_ts("Can't rename %s to %s errno %d", from, to, errno);
		exit(EXIT_FAILURE);
	}
}
...
static void rename_force(const char *from, const char *to) {
	if (access(to, R_OK) == 0) {
		msg_ts("Removing %s", to);
		if (my_delete(to, MYF(MY_WME))) {
			msg_ts("Can't remove %s, errno %d", to, errno);
			exit(EXIT_FAILURE);
		}
	}
	rename_file(from,to);
...
store_binlog_info(const char* filename, const char* name, ulonglong pos)
{
	...
	if (!fp) {
		msg_ts("mariabackup: failed to open '%s'", filename);
		return(false);
	}
...
static std::string read_file_as_string(const std::string file) {
	...
	if (!f) {
		msg_ts("Can not open %s", file.c_str());
	}
...
static void delete_file(const std::string& file, bool if_exists = false) {
	...
	if (my_delete(file.c_str(), MYF(MY_WME))) {
		msg_ts("Can't remove %s, errno %d", file.c_str(), errno);
		exit(EXIT_FAILURE);
	}
}
...
static void rename_table_in_prepare(const std::string &datadir, const std::string& from , const std::string& to,
	...
	if (file_exists(dest)) {
		...
		if (!file_exists(ren2)) {
			msg_ts("ERROR : File %s was not found, but expected during rename processing\n", ren2.c_str());
			ut_a(0);
		}
		tmp = to + "#";
...
xtrabackup_prepare_func(char** argv)
{
	...
	{
		msg_ts("can't my_setwd %s", xtrabackup_real_target_dir);
		return(false);
	}
	msg_ts("cd to %s", xtrabackup_real_target_dir);
	...
	*/
	...
	if (!xtrabackup_read_metadata(metadata_path)) {
		msg_ts("Error: failed to read metadata from '%s'\n",
		    metadata_path);
		return(false);
	}
	...
	if (!strcmp(metadata_type, "full-backuped")) {
		if (xtrabackup_incremental) {
			msg_ts("error: applying incremental backup "
			    "needs a prepared target.");
			return(false);
		}
		msg_ts("This target seems to be not prepared yet.");
	} else if (!strcmp(metadata_type, "log-applied")) {
		msg_ts("This target seems to be already prepared.");
	} else {
		msg_ts("This target does not have correct metadata.");
		return(false);
	}
	...
	if (!ok) {
		msg_ts("error: This incremental backup seems "
	...
	if (xtrabackup_incremental) {
		...
		if (err != DB_SUCCESS) {
			msg_ts("mariabackup: error: xb_data_files_init() failed "
			    "with error %s\n", ut_strerr(err));
			goto error_cleanup;
		}
	...
	msg_ts("Starting InnoDB instance for recovery.");

	msg_ts("mariabackup: Using %lld bytes for buffer pool "
	...
	if (ok) {
		...
		if (mach_read_from_4(TRX_SYS_MYSQL_LOG_INFO
			...
			const char* name = reinterpret_cast<const char*>(
				...
				+ sys_header);
			msg_ts("Last binlog file %s, position %llu", name, pos);
	...
	if (ok) {
		...
		if (!xtrabackup_write_metadata(filename)) {
			...
			msg_ts("mariabackup: Error: failed to write metadata "
			    "to '%s'", filename);
			ok = false;
		} else if (xtrabackup_extra_lsndir) {
			sprintf(filename, "%s/%s", xtrabackup_extra_lsndir, XTRABACKUP_METADATA_FILENAME);
			if (!xtrabackup_write_metadata(filename)) {
				msg_ts("mariabackup: Error: failed to write "
						...
						is not granted */
{
	if (!has_privilege(granted_priv,
		...
		msg_ts("%s: missing required privilege %s on %s.%s",
...
xb_init()
{
	...
	if (opt_slave_info
		&& opt_no_lock
		&& !opt_safe_slave_backup) {
		msg_ts("Error: --slave-info is used with --no-lock but "
	...
	if (xtrabackup_backup && opt_rsync)
	{
		...
		{
			msg_ts("Error: --rsync doesn't work with --stream\n");
			return(false);
		}
		bool have_rsync = IF_WIN(false, (system("rsync --version > /dev/null 2>&1") == 0));
		if (!have_rsync)
		{
			msg_ts("Error: rsync executable not found, cannot run backup with --rsync\n");
			return false;
		}
	}
	...
	if (n_mixed_options > 1) {
		msg_ts("Error: %s and %s are mutually exclusive\n",
	...
	*argv_server = argv;
	...
	for (int i = 2 ; i < argc ; i++) {
		...
		if (optend - argv[i] == 15 &&
			...
			msg_ts("mariabackup: Error: --defaults-file "
			...
			msg_ts("mariabackup: Error: --defaults-extra-file "
			    "must be specified first on the command "
			    "line\n");
			exit(EXIT_FAILURE);
		}
...
static int main_low(char** argv)
{
	...
	if (!xtrabackup_print_param && !xtrabackup_prepare
		...
		if (!xtrabackup_print_param)
			usage();
		msg_ts("mariabackup: Error: Please set parameter 'datadir'");
		return(EXIT_FAILURE);
	}
	...
	if (xtrabackup_backup && xtrabackup_incremental) {
		...
		if (error) {
			msg_ts("mariabackup: value '%s' may be wrong format for "
		...
		if (!xtrabackup_read_metadata(filename)) {
			msg_ts("mariabackup: error: failed to read metadata from "
			    "%s", filename);
			return(EXIT_FAILURE);
		}
	...
	if (xtrabackup_stream && !xtrabackup_backup) {
		msg_ts("Warning: --stream parameter is ignored, it only works together with --backup.");
	}
	...
	print_version();
	if (xtrabackup_incremental) {
		msg_ts("incremental backup from " LSN_PF " is enabled.",
		    incremental_lsn);
	}

	if (xtrabackup_export && innobase_file_per_table == FALSE) {
		msg_ts("mariabackup: auto-enabling --innodb-file-per-table due to "
		    "the --export option");
